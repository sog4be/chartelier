...
- [13. 代表テストシナリオ詳細（抜粋）](#13-代表テストシナリオ詳細抜粋)
  ...
  - [13.2 「LLMタイムアウト→パターン選択エラー」](#132-llmタイムアウトパターン選択エラー)
...
- MCP準拠の**ステートレス可視化ツール**として、**9パターン制約**と*フェイルセーフ（チャート選択フォールバック）*を中心に品質担保する。
...
- **Phase 1（必須/P0）**: MCP、9パターン、エラーハンドリング、パターン選択エラー処理、PNG/SVG出力、データ制約/サンプリング、監視・運用IF。
...
- **FR-022**: **3×3（9パターン）制約内で決定。パターン選択失敗時はエラー**。
...
| 要素 | 観点 | 説明 |
| --- | --- | --- |
| パターン | P01..P32（表定義） | 9パターンの用途に合致。LLM失敗時は**エラー**。 |
...
| FR-022（9パターン） | UT-PS-\*, ST-PAT-0x、E2E-002 |
...
| **UT-PS-005** | LLM失敗 | タイムアウト/無効応答（モック） | **PatternSelectionError** |
...
| **IT-FB-040** | 途中失敗→縮退 | ChartSelector失敗 | 既定テンプレで継続成功 |
...
| **ST-ERR-005** | LLM障害 | LLMClientモックで5xx/timeout | E422エラー＋意図明確化のヒント |
...
| **E2E-002** | US-002 | 明確なエラー情報と代替提案 | 欠損列や型不整合を意図的に投入 | E422＋修正ヒント、クエリ改善提案 |
...
| **NFT-FB-011** | フォールバック | PNG失敗注入 | 100%でSVGに縮退 |
...
| リスク | 兆候 | 迂回策 |
| --- | --- | --- |
| LLMレイテンシ変動 | P95悪化 | サービス一時停止またはマニュアル運用、プロンプト短縮 |
...
### 13.2 「LLMタイムアウト→パターン選択エラー」

1. LLMClientをtimeoutモック
2. PatternSelectorでPatternSelectionError送出
3. E422_UNPROCESSABLEエラーを返却
**期待**: エラーメッセージに意図明確化のヒントを含む

...
- [11. Monitoring & Operations](#11-monitoring--operations)
  ...
  - [11.6 代表的インシデントと対処](#116-代表的インシデントと対処)
...
現在のMVPでは、実装の簡潔性と品質保証の観点から、補助要素は**target_line（目標値線）のみ**をサポートします。

| カテゴリ | 要素ID | 要素名 | 説明 | 主な用途 |
| --- | --- | --- | --- | --- |
| 参照線 | target_line | 目標値線 | ビジネス目標やKPIを示す基準線 | 目標達成度、パフォーマンス評価 |

将来的な拡張候補として以下の要素を検討していますが、現時点では実装対象外です：
- データ強調（highlight、annotation、color_coding）
- その他の参照線（mean_line、median_line、threshold）
- トレンド表示（regression、moving_avg、forecast）
...
- グラフあたり最大1要素までに制限（現在はtarget_lineのみ）
...
- **ChartSelector**
    ...
    パターンとデータ特性から最適なグラフ種類と補助要素を選択。利用可能なグラフタイプの候補抽出、LLMを活用した最適なチャートタイプの決定、補助要素（target_lineのみ）の選択、フォールバック時の標準グラフ（折れ線）への自動選択を担当。
